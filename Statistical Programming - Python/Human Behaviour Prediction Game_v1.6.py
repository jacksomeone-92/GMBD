...
def main():
    ...
    if select_difficulty == 2:
        ...
        player_previous_move = None #set previous move to none for the first turn
        for turn in range(moves):             
            ...
            if turn == 0: #first move has no previous one
            ...
            if player_move_2 == computer_move:
               MS = MS + 1
               print("player = %d machine = %d - Computer wins!" % (int(player_move_2), computer_move)) #pick the last move from play_move_dif
               print("You: %d Computer: %d" % (PS, MS))
            else:
               PS = PS + 1
    ...
    continue_game = int(input("Would you like to start a new game (1) or quit (2) "))
    if continue_game == 1:
        main()
        ...
        print("Sorry, I did not understand, please reply with either '1' or '2'")
